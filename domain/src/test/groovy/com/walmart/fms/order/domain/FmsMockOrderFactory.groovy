...
class FmsMockOrderFactory {
    ...
    static String storeWebOrderID = UUID.randomUUID().toString()
    static String sourceOrderId = UUID.randomUUID().toString()
    static Vendor vendor = Vendor.UBEREATS
    static String CANCELLED_REASON_CODE = "Code12345"

    static FmsOrder open_order() {
        return new FmsOrder("INITIAL")
    }


    static FmsOrder fms_order_with_status(String orderState) {
    ...
    static FmsOrder a_marketplace_order_without_items() {
        FmsOrder testFmsOrder = FmsOrder.builder()
                .vertical(Vertical.ASDAGR)
                .tenant(Tenant.ASDA)
                .storeId("4401")
