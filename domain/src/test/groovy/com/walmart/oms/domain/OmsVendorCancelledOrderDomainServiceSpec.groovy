...
class OmsVendorCancelledOrderDomainServiceSpec extends Specification {
    OmsVendorCancelledOrderDomainEventPublisher omsVendorCancelledOrderDomainService

    DomainEventPublisher omsDomainEventPublisher = Mock()

    def setup() {
        omsVendorCancelledOrderDomainService = new OmsVendorCancelledOrderDomainEventPublisher(omsDomainEventPublisher)
    }

    def "Test for vendor cancelled order"() {
        when:
        omsVendorCancelledOrderDomainService.sendCancelOrderEvent(mockOmsOrder())
        then:
        1 * omsDomainEventPublisher.publish(_ as DomainEvent, _ as String) >> { DomainEvent domainEvent, String destination ->
            assert destination == "OMS_ORDER_CANCELLED"
        }
    }
