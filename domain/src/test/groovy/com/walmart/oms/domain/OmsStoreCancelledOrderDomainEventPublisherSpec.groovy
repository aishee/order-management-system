...
class OmsStoreCancelledOrderDomainEventPublisherSpec extends Specification {
    OmsStoreCancelledOrderDomainEventPublisher omsStoreCancelledOrderDomainService

    DomainEventPublisher omsDomainEventPublisher = Mock()

    def setup() {
        omsStoreCancelledOrderDomainService = new OmsStoreCancelledOrderDomainEventPublisher(omsDomainEventPublisher)
    }

    def "Test for store cancelled order"() {
        when:
        omsStoreCancelledOrderDomainService.sendCancelOrderEvent(mockOmsOrder())
        then:
